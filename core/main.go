...
// FSClient is a interface that makes it easy to replace the built-in Client with your own version.  You can
// pass an FSClient into core.Main() from main.main() and your implementation will be instead of the default.
type FSClient interface {
	DoReq(req *http.Request) (io.ReadCloser, error)
	ExportList(start time.Time) ([]client.ExportMeta, error)
	ExportData(id int, modifyReq ...func(r *http.Request)) (client.ExportData, error)
}

type ExportProcessor interface {
	Export(warehouse.Warehouse, []string, FSClient, []client.ExportMeta) (int, error)
}

// we have twe different implementations of ExportProcessor that can be switched out based on config file settings.
// for now, these are just wrappers around functions with the appropriate name.
type Individually struct {
}
func (i *Individually) Export(wh warehouse.Warehouse, s []string, fs FSClient, meta []client.ExportMeta) (int, error) {
	return ProcessFilesIndividually(wh,s,fs,meta)
}
type ByDay struct {
}
func (b *ByDay) Export(wh warehouse.Warehouse, s []string, fs FSClient, meta []client.ExportMeta) (int, error) {
	return ProcessFilesByDay(wh,s,fs,meta)
}
...
// MustGetConfig returns either the (newly read) current config file or it aborts with an error.
// This is used by main to retrieve the config file before creating the FSClient.
...
func ProcessExportsSince(wh warehouse.Warehouse, fs FSClient, tableColumns []string, since time.Time, exportProcessor ExportProcessor) (int, error) {
	...
	return exportProcessor.Export(wh, tableColumns, fs, exports)
}
...
func ProcessFilesIndividually(wh warehouse.Warehouse, tableColumns []string, fs FSClient, exports []client.ExportMeta) (int, error) {
...
func ProcessFilesByDay(wh warehouse.Warehouse, tableColumns []string, fs FSClient, exports []client.ExportMeta) (int, error) {
	if len(exports) == 0 {
		return 0, nil
	}
	if conf.SaveAsJson {
...
func getExportData(fs FSClient, bundleID int) (client.ExportData, error) {
...
func WriteBundleToCSV(fs FSClient, bundleID int, tableColumns []string, csvOut *csv.Writer, wh warehouse.Warehouse) (numRecords int, err error) {
...
func WriteBundleToJson(fs FSClient, bundleID int, filename string) (bytesWritten int64, err error) {
	stream, err := getExportData(fs, bundleID)
	if err != nil {
		log.Printf("Failed to fetch bundle %d: %s", bundleID, err)
		return 0, err
...
func Main(fsc FSClient) {

	var exportProcessor ExportProcessor
	exportProcessor = &Individually{}
	if conf.GroupFilesByDay {
		exportProcessor = &ByDay{}
	}
	...
	for {
		...
		numBundles, err := ProcessExportsSince(wh, fsc, tableColumns, lastSyncedRecord, exportProcessor)
		if BackoffOnError(err) {
			continue
		}
